<?PHP
	...
	Autor: DanielR92
	;Title: Add 2 Groupe
	;Desc: Set User to a Group when he is in a specific Channel
	**/
	class InfoClass { }

	$plugin_reflection = new ReflectionClass('InfoClass');
	//return $plugin_reflection->getDocComment();
	...
	require_once('../../webinterface/nav.php');
	...
	$vokale = array("/**", "**/");
	$onlyconsonants = str_replace($vokale, "", $plugin_reflection->getDocComment());
	
	$onlyconsonants =  explode( ';', $onlyconsonants);
	foreach($onlyconsonants as $value)
	{
		echo $value . "<br>";
	}
	
	if(isset($err_msg)) error_handling($err_msg, $err_lvl); ?>
